...
DataRow buildBalanceSheetDataRow(int index, BalanceSheetReportModel tblData, final List<Map<dynamic, dynamic>> dropDownList, [BuildContext? context]) {
  return DataRow(
    ...
    cells: [
      ...
      buildDataCell(200, '${((tblData.layerPosition == 1 && tblData.nature != "Total" )|| tblData.layerPosition == 2) ? "" : tblData.debit?.abs()}', TextAlign.end, tblData.layerPosition!, tblData.nature!),
      buildDataCell(200, '${tblData.accountGroupName2}', TextAlign.start, tblData.layerPosition!, tblData.nature!),
      buildDataCell(160, '${(tblData.layerPosition == 1 || tblData.layerPosition == 2) ? tblData.credit?.abs() : tblData.credit?.abs()}', TextAlign.end, tblData.layerPosition!, tblData.nature!),
    ],
  );
}
